...
/**
 * Sube la foto de un empleado (Fase 10 — el círculo del selector de
 * arriba del panel). Reutiliza el MISMO bucket `business-logos` en vez de
 * crear uno nuevo: las policies de Storage de ese bucket solo miran el
 * PRIMER segmento de la ruta (`business_id`), así que guardar en
 * `${businessId}/employees/...` ya queda autorizado igual que el logo —
 * sin ninguna migración nueva de Storage.
 */
export async function uploadEmployeePhoto(
  ...
  const path = `${businessId}/employees/${employeeId}-${Date.now()}.${extension}`;
  ...
  const { error: updateError } = await (client.from("employees") as any)
